...
**I simulated a code path instead of running it, "found" a bug that a guard had already covered since July, and wrote a ledger entry about it.**

Checking whether econ's rewritten SEO pages were safe to publish, I queried catalog.db directly:
`SELECT source_id, MIN(start_date), MAX(end_date) FROM series GROUP BY source_id`. It showed 35
sources with impossible end dates — eurostat 9999, sec_edgar 6016, hagstofa 3005 — and I concluded
the pages were one regeneration away from publishing "1947–9999" as fact. I told Ahmed it was "a
clear no, not as-is", wrote a guard, tested the guard, and logged the whole thing as a mistake
caught.

None of it was true. `sane_date()` sits between that SQL and the page, has since the initial
public release on 2026-07-02, and drops any year below 1000 or above `MAX_SANE_YEAR`. The pages in
the working tree contain **zero** impossible ranges — I could have grepped for `9999` in the
generated HTML in one command, and eventually did, which is how this surfaced.

My simulation reproduced the QUERY and skipped the FUNCTION. It is the same error as R222 (I
measured my own staging copy and reported it as production) and R227 (a check whose predicate was
broken, not the subject): I reconstructed what I believed the code did rather than observing what
it produced, then trusted my reconstruction over the artifact sitting on disk.

The tell I walked past: the working-tree pages already read `statfin ... from 1900` and
`hagstofa ... from 1900` — the exact "end date dropped" shape my "fix" was supposed to introduce.
The guard's output was in front of me, and I read it as evidence of the disease rather than of the
cure.

**What survives.** Two things, both verified against the artifact rather than a model of it:
* the series COUNTS on four pages are stale — noaa claims 10 where catalog.db and the live API
  both say 3,135,873, likewise census 22 vs 2,993, fed_board 21 vs 52,322, ilostat 80 vs 3,305.
  Regenerating fixes them. 112 of 117 counts already matched exactly.
* my change is a mild net improvement, not a fix: the corroboration test recovers two TRUE start
  years that `sane_date`'s blunt floor was discarding (scb 1749, stat_slovenia 1857). The
  plausibility ceiling I added is dead code behind `sane_date`'s tighter one.

**The rules.**
1. Do not simulate a pipeline you can RUN. Executing the generator into a temp directory took one
   script and settled in seconds what an afternoon of reasoning got backwards.
2. Check the OUTPUT before theorising about the input. `grep 9999 catalog/site/*.html` was the
   whole investigation.
3. Before "fixing" a defect, find the commit that introduced it. `git log -S"def sane_date"`
   dated the guard to a month before I claimed the bug existed.
4. When existing output already has the shape your fix would produce, that is evidence the guard
   EXISTS — not evidence it is needed.
...
Sweeping every live cloud source for the R243 shape (accumulate-then-merge under a 45-minute
kill), I classified them by counting `merge_and_write`. Three answers in a row, each from a
tool one level too shallow for the claim:

1. **`grep -c merge_and_write`.** unhcr came back 2 — "truncates, mild". One of those hits is
   in the module DOCSTRING. A grep counts text.
2. **An AST walk counting call sites.** Now unhcr, hagstofa, stat_latvia, stat_slovenia and
   insee_bdm all came back exactly 1, and I wrote into a PUSHED commit message that "every one
   of them is DISCARD-on-kill, the worst class". But a call SITE is not an EXECUTION: one merge
   inside a `for` loop runs per sub-unit and preserves progress.
3. **Checking whether the call sits inside the sweep loop.** Only then: unhcr and bcb merge
   after the loop (discard); hagstofa, stat_latvia, stat_slovenia and insee_bdm merge inside it
   (truncate). Four of the five I had just labelled worst-class were not.

Nothing shipped wrong — the fixes are right either way, and bounding a truncating source is
still worth it for throughput. What shipped wrong was the CLAIM: in a commit message, in the
confident register, about four sources I had not opened.

Each tool answered a question ADJACENT to mine. "How many times does this string appear" is not
"how many merges are there"; "how many merge calls are written" is not "how many times does a
merge run". I accepted each because it was precise, and precision reads as correctness.

**The rules.**
1. Match the tool to the CLAIM, not to the keyword. A claim about runtime behaviour ("this
   discards on kill") cannot be established by a static count of occurrences — you need the
   control flow the call sits in.
2. A number is not evidence for the proposition it was gathered for. Ask what would have to be
   true for this count to mean what I am about to say, then check THAT.
3. Never put a class-wide claim in a commit message from a sweep you have not spot-checked by
   hand. I read unhcr and bcb, then generalised to three modules I never opened.
4. When a cheap check and an expensive check disagree, the cheap one is not "close enough" —
...
**What happened.** `tools/audit_schedule_coverage.py` is the instrument behind the figure I
report every cycle. It defined SCHEDULED as `registry live:true` UNION the updater-heavy matrix
UNION sec-edgar-daily. There is a fourth mechanism: `tools/run_local_heavy.ps1` asks
`tools/_list_local_sources.py` which sources to run, and that selects on
`run_location == "local"` **regardless of `live`**. Nine sources are refreshed every ~20h on the
workstation without ever setting `live:true`, and the audit called every one of them unscheduled:
bis, bls, census, eia, faostat, istat, oecd, statcan, vdem.

    120 of 217 sources / 9,496,558 series   (I had been reporting 112 / 9,479,138)

The proof was in front of me the whole time. The same night I read the workstation log —
"registry routes 17 source(s) to this machine: … census …" — and watched census merge +8,457
rows, I also reported census as having no update path. Two facts, one session, never compared.

**Why it lasted.** The definition came from the standing instruction, so it felt authoritative
rather than like a claim to test. But an instruction that names three mechanisms is a
description of the system as understood WHEN IT WAS WRITTEN, and the fourth mechanism was added
later (the cloud-infeasible sources were routed to the workstation on 2026-07-31). A definition
does not update itself when the system grows a new limb.

**The rule.** The measurement instrument is itself a claim, and it decays. When a number comes
from an enumeration of mechanisms, periodically re-derive the enumeration from the SYSTEM —
every workflow, every scheduled script, every launcher — instead of from the definition you were
handed. Concretely: what starts a run? Answer that from the repo, not from memory. And when a
log contradicts a report in the same session, that contradiction is the finding.

**One thing it got right, worth preserving.** The tool already refuses to count matrix members
with no adapter — "membership in a schedule is not the ability to run" — after a measured 2026-07-30
case where four matrix jobs ran green with "0 unit(s) processed". I applied the same caution to
the new path, so `cbs_nl` and `gus_dbw` come back STRANDED rather than scheduled. Adding a
mechanism to a counter is exactly where the temptation to inflate lives.
...
**What happened.** Reviewing dst's run history I found its reported observation count fall from
**9,198,885** (2026-06-24) to **231,035** (2026-07-25) — a 97.5% drop that `merge`'s never-shrink
guard is supposed to make impossible. I nearly logged it as data loss.

The store was fine. Counting the parquet metadata directly: **9,220,012 rows** across 707 files,
i.e. slightly MORE than June. The 231,035 was a false number that dst computed about itself.

**The mechanism, and it is the R36/R261 listing bug with a price tag.** `dst._total_rows()`
read through `blob` but LISTED with `os.listdir` behind an `os.path.isdir` guard. On a runner
under `AQUEDUCT_BACKEND=r2` that directory DOES exist — `blob.write_table_atomic` keeps a local
copy as a scratch mirror for the same-run derive — but it holds ONLY the files this run wrote.
So the guard passed, the listing succeeded, and the sum covered a handful of freshly-written
files instead of the 706-object store. Not an error, not an empty result: a plausible number,
off by 40x, written into the runs table and the daily digest.

**Why this is worse than the empty-listing case.** R261 was about `[]` reading as a fact. This
is the same defect producing a NON-empty wrong answer, which is strictly harder to catch —
`231,035` looks like data, invites a data-loss investigation, and would have had me hunting a
merge bug that does not exist. An empty result at least looks like nothing happened.

**The rule.** When a listing is wrong, audit every NUMBER derived from it, not just the missing
work. Row counts, coverage percentages, "N series refreshed" — each becomes a confident false
statement about the system, and those outlive the bug because they get recorded. And before
concluding "data loss" from a reported figure, count the store: a metric about our own size is
a claim like any other, and cheaper to verify than to believe.

Fixed by the same commit that routed dst's listing (c40bcd04); the correction is that
`blob.list_parquets` under r2 enumerates all 706 R2 objects rather than the scratch mirror.
...
**What happened.** Task #65 read: "bea: 912,990 series are in the store and DARK — not 17,459;
and the fetcher reads the wrong store." I fixed the second clause — `_tree_frontier` walked the
tree with a raw local glob, so under `AQUEDUCT_BACKEND=r2` it found nothing and the frontier
fell back to a grouped file holding under 2% of the series — verified it (None -> 2026-04-01
against the live store), and marked the task **completed**.

The headline was the FIRST clause, and I never touched it. bea still has 240 catalogued series
against a store of 913,230. Tonight's run said so plainly: "[bea] MIGRATED 240 legacy series
(44,499 obs)", then finished in one second — maintaining a 240-series surface over a
913,230-series store. I only noticed because I happened to read the live log for another reason.

**Why it happened.** The clause I fixed was the one I had spent an hour inside. Its detail was
vivid, the other clause was a sentence I had read once, and "done" attached to the work I
remembered doing rather than to the task as written. The task title even led with the part I
skipped.

**The rule.** Before closing a task, re-read its own text and account for every claim in it
separately. If a task states two facts, closing it asserts both are handled — and a title that
begins "912,990 series are DARK" is not discharged by a frontier fix. Where only part is done,
say which part in the status line, so the next reader inherits the gap instead of the
impression.

**Corollary about scope.** The two halves were not the same kind of work at all: one was a code
bug I could fix and prove in an hour, the other is gated on a capacity decision (#45, D1 at
7.87/10 GB against ~75.7 GB of dark stores). A task mixing a bug with a decision will always
tempt me to close it on the bug.
...
**R350 CORRECTION (same session, same day):**
**I "discovered" a phantom grain that was a deliberate, documented design — by testing key EQUALITY against a resolver built precisely because equality cannot work.**
Measuring `imf_imts_direct` before touching it, I found its catalogue holds 2,937 ids shaped
`IMTS:ABW.A.MG_CIF_USD` (country·freq·indicator) while its store holds 472,234 keys shaped
`IMTS:NIC.ALB.M.TBG_USD.IMTS` (country·**counterpart**·freq·indicator). I intersected the two
sets, got 0 of 2,937, ran a control to prove the absence was real, and wrote a ledger entry
declaring every catalogued row an offer to serve something that cannot be produced.
The intersection was correct. The conclusion was backwards. `git log` on the source — one command,
which I ran only afterwards — shows commit 51273342, *"2,937 table rows for 472,234 series,
resolver + derive + flip"*, and its body states the design in full: TABLE grain chosen by the same
D1 arithmetic I then re-derived as though it were an open question, "the census/usda/statcan/PxWeb
precedent", each table row serving every series inside its table's CSV. It even names my error in
advance: *"bespoke prefix+suffix predicate (counterpart sits MID-key, so the PxWeb pure-prefix
_FLOW_GRAIN cannot match)"* — with two tests covering the wrong-table traps.
So a catalogued id is NOT supposed to equal a store key here. I modelled the resolution rule as
equality and tested my model, when the resolver is a file I could have called. That is R231 in a
new costume: simulate what you could execute. The control I was so pleased with made the wrong
measurement trustworthy, not the right one — a passing control certifies the instrument, never
the question.
The tell I walked past: 2,937 is not a fraction of 472,234, it is 245 countries × 4 frequencies ×
3 indicators. A number that factors that cleanly is a designed grain, not a truncation.
1. `git log --oneline -- <path>` and read the commit BODY before declaring anything about a
   source's design. The reasoning is written down; treating it as unknown re-derives it badly.
2. When a catalogue and a store disagree in shape, find the RESOLVER and call it. Set
   intersection tests the hypothesis "ids equal keys", which coarse-grain serving explicitly
   denies — census, usda, statcan and every PxWeb source work this way.
3. A count that factors into obvious dimensions (245 × 4 × 3) is a deliberate grain. Factor the
   number before theorising about it.
4. A passing control validates the instrument, not the question. Mine proved my set-membership
   worked; it could not tell me set membership was the wrong test.
...
### R351 — (renumbered from a stale '## R237') duplicated a concurrent session's fix
...
**R348 RECURRENCE (2026-08-05):** hours after writing R348 I backgrounded a dispatch chain with a bare `&` (R89's shape); the dispatch survived, the watcher died. Compensating control: verify the action's post-state immediately — the run list showed the dispatch fired. Never trust the launch line of a command containing `&` or a pipe.

### R352 — a concurrent session corrected the ledger from a stale buffer and deleted 118 entries

**What happened.** Two sessions worked this repo concurrently (data pipelines never collided — task #104's ownership metadata held). The sibling appended a ledger entry cleanly (8cbc0cc, +50, header `## R236` — two hashes, invisible to the `### R` count), then REWROTE the file to correct itself (d99b515: 34 insertions, 4,924 DELETIONS) from a buffer predating 118 entries; its next append (97c1f12) sat on the mutilated file. The numbers were the tell: 'R236' landed on a file whose newest entry was R349 — and the TRUE ledger already had an R236 from weeks ago, which this reconstruction's own first attempt then mangled via an unanchored substring replace ('### R236' contains '## R236') — the fix's fix needed a line anchor. Substring tests are not membership tests (R112), including on header lines.
